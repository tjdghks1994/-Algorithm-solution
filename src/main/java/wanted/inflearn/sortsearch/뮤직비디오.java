...
public class 뮤직비디오 {
     ...
     * 예시 : 입력 - 9 3                    출력 - 17
     *            1 2 3 4 5 6 7 8 9
     *       설명 - 3개의 DVD 용량이 17분짜리이면 (1,2,3,4,5) (6,7) (8,9) 이렇게 3개의 DVD로 녹음을 할 수 있다.
     *             17분 용량보다 작은 용량으로는 3개의 DVD에 모든 영상을 녹화할 수 없다.
     */

    // capacity 로 모든 DVD를 녹화하려면 필요한 DVD 개수 구하기
    public static int count(int[] arr, int capacity) {
        int cnt = 1;    // DVD 개수
        int sum = 0;    // 현재 DVD 녹음 용량

        for (int x : arr) {
            // 현재 DVD 용량 + x 값이 capacity 값을 초과하는 경우
            if (sum + x > capacity) {
                cnt++;      // DVD 개수 추가
                sum = x;    // 현재 DVD 용량을 x 값으로 초기화
            } else {    // sum + x 값이 capacity 값보다 작은 경우 ( 현재 DVD에 녹음이 가능한 경우 )
                sum += x;   // 현재 DVD 용량에 x 값을 추가
            }
        }
    ...
    public static void solution(int m, int[] arr) {
        ...
        while (lt <= rt) {
            // DVD 용량
            int mid = (lt + rt) / 2;
            // 현재 DVD 용량으로 m개 이하의 DVD 갯수로 모든 영상을 녹화할 수 있는 경우
            if (count(arr, mid) <= m) {
                answer = mid;
                rt = mid - 1;
            } else {    // 현재 DVD 용량으로는 m개 이하의 DVD 갯수로 모든 영상을 녹화할 수 없는 경우
                lt = mid + 1;
            }
        }
